(function () {
    ...
    var getToolbarItems = function (editor) {
      return editor.getParam('link_toolbar', 'cardlink normallink insidelink | deletelink copylink openlink');
    };
    ...
    var isTapdLink = function (elm, editor) {
      return false;
    };
    ...
    var updateLink = function (editor, anchorElm, text, linkAttrs) {
      ...
      if (anchorElm.getAttribute('data-link-mode') === 'unLinkMode') ; else if ( anchorElm.getAttribute('is-tapdlink')) {
    ...
    var linkDomMutation = function (editor, attachState, data, elm) {
      var selectedElm = elm || editor.selection.getNode();
    ...
    var link = function (editor, attachState, data, elm) {
      if (elm === void 0) {
        elm = null;
      }
    ...
    var dialog = {
      ...
      show: function (data, configs) {
        ...
        setTimeout(function () {
          _this.$('.form-item__href').focus();
        }, 0);
        document.querySelector('.' + this.dialogClass).style.display = 'block';
        this.applyData(data);
      ...
      hide: function () {
        document.querySelector('.' + this.dialogClass).style.display = 'none';
      ...
      onEnsure: function () {
        var submit = this.configs.submit;
        if (submit) {
          if (submit()) {
            this.hide();
          }
      ...
      applyData: function (data) {
        var _this = this;
        var temp = Object.assign({}, this.defaultData, data);
        if (!temp.href) {
          temp.href = this.defaultData.href;
        }
        Object.keys(temp).forEach(function (key) {
          var el = _this.$('.form-item__' + key);
          el.value = temp[key];
        });
      },
      getData: function () {
    ...
    var api = {
      ...
      data: {},
      editor: null,
      show: function (data, editor) {
        var _this = this;
        var _a = data.initialData, text = _a.text, title = _a.title, target = _a.target, link = _a.link;
        ...
        dialog.show({
          ...
          noText: this.noText,
          submit: function () {
            var beforeInsetLink = editor.getParam('before_insert_link', function () {
              return true;
            });
      ...
      getData: function () {
        var _a = dialog.getData(), text = _a.text, title = _a.title, target = _a.target, href = _a.href;
        if (this.noText) {
          return {
            target: target,
            title: title,
    ...
    var handleSubmit = function (editor, info, elm) {
      return function (api) {
        ...
        DialogConfirms.preprocess(editor, changedData).then(function (pData) {
          link(editor, attachState, pData, elm);
        });
        api.close();
      };
    };
    ...
    var getInitialData = function (info, defaultTarget, editor) {
      ...
      return {
        ...
        text: editor.selection.getNode().getAttribute('linktext') || anchor.text.getOr(''),
    ...
    var open = function (editor) {
      ...
      data.then(function (info) {
        ...
        var onSubmit = handleSubmit(editor, info, elm);
        return makeDialog(info, onSubmit, editor);
      }).then(function (spec) {
        api.show(spec, editor);
      });
    };
    ...
    var copyLink = function (editor) {
      return function () {
        var url = getSelectedLink(editor).href;
        ...
        if (url !== null) {
          editor.fire('linkCopySuccess');
        }
      };
    };
    var deleteLink = function (editor) {
      return function () {
        var link = getSelectedLink(editor);
        editor.dom.remove(link);
        editor.editorContainer.querySelectorAll('.tox-silver-sink')[0].remove();
        editor.fire('linkDelectSuccess');
      };
    };
    ...
    var register = function (editor) {
      ...
      editor.addCommand('mceCardMode', function () {
        editor.fire('drawCardLink');
      });
    };
    ...
    var setupButtons = function (editor) {
      ...
      editor.ui.registry.addButton('copylink', {
        tooltip: 'Copy Link',
        onAction: copyLink(editor),
        icon: 'copy-link'
      });
      editor.ui.registry.addButton('deletelink', {
        tooltip: 'Delete Link',
        icon: 'link-remove',
        onAction: deleteLink(editor)
      });
    };
    ...
    var setupContextToolbar = function (editor) {
      var linkToolbar = getToolbarItems(editor);
      editor.ui.registry.addContextToolbar('link', {
        items: linkToolbar,
        predicate: function (elem) {
          var isShowEntityLink = isLink(elem) && isTapdLink() && elem.getAttribute('is-tapdlink') === 'true';
    ...
    var setupContextPopover = function (editor) {
      editor.ui.registry.addPopover('link', {
        predicate: function (elem) {
          var isShowEntityLink = isLink(elem) && isTapdLink();
          return isShowEntityLink;
        },
        position: 'node',
        scope: 'node',
