(function () {
    ...
    var makeDialog = function (helpers, editor) {
      return function (info) {
        ...
        if (!/^\d+$/.test(info.image.width)) {
          var dimensions = helpers.imageComputedSize(editor);
          info.image.width = dimensions.width || info.image.width;
          info.image.height = dimensions.height || info.image.height;
        }
        return {
          title: 'Resize',
          size: 'normal',
    ...
    var imageComputedSize = function (editor) {
      return function () {
        var imgElm = getSelectedImage(editor);
        var width = String(Math.round(+window.getComputedStyle(imgElm).getPropertyValue('width').replace('px', '')) || '');
        var height = String(Math.round(+window.getComputedStyle(imgElm).getPropertyValue('height').replace('px', '')) || '');
        return {
          width: width,
          height: height
        };
    ...
    var Dialog = function (editor) {
      var helpers = {
        ...
        imageComputedSize: imageComputedSize(editor),
      ...
      var open = function () {
        collect(editor).then(makeDialog(helpers, editor)).then(editor.windowManager.open).then(function () {
          document.querySelector('.tox-dialog').classList.add('cherry-image--dialog');
        });
      };
      return { open: open };
    };
    ...
    var register$1 = function (editor) {
      var doUpload = function () {
        ...
        if (filePickerCallback) {
          filePickerCallback(function (url) {
            if (url) {
              var clientWidth_1 = editor.editorContainer.clientWidth;
              var dom_1 = document.createElement('img');
              dom_1.src = url;
              dom_1['data-mce-src'] = url;
              dom_1.onload = function () {
                if (dom_1.width > clientWidth_1 * 0.8) {
                  dom_1.style.width = '80%';
                }
                editor.insertContent(dom_1.outerHTML);
              };
              dom_1.onerror = function () {
                editor.insertContent(dom_1.outerHTML);
              };
            }
          }, '', { filetype: 'image' });
      ...
      var uuid = 0;
      var doInputUrl = function () {
        var uid = uuid += 1;
        var innerHTML = '<input type="text" id="imageUrl" class="network-image-' + uid + '" placeholder="http://">' + '<p class="plugin_image--text">\u65e0\u8bbf\u95ee\u6743\u9650\u7684\u56fe\u7247\u53ef\u80fd\u5bfc\u81f4\u65e0\u6cd5\u663e\u793a\u3002</p>';
        editor.windowManager.open({
          ...
          onSubmit: function () {
            var img = document.querySelector('.network-image-' + uid);
            var url = !!img ? img.value : '';
      ...
      editor.ui.registry.addMenuButton('image', {
        ...
        fetch: function (callback) {
          callback([
            {
              type: 'togglemenuitem',
              text: global$5.translate('Upload Local File'),
              onAction: function () {
                doUpload();
              }
            },
            {
              type: 'togglemenuitem',
              text: global$5.translate('Net Image'),
              onAction: function () {
                doInputUrl();
              }
            }
