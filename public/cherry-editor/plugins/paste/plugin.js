(function () {
    ...
    var hasWXWorkImage = function (content) {
      return hasContentType(content, 'text/html') && /file:\/\/\/.*WXWork/.test(content['text/html']);
    };
    var isPasteSingleImage = function (clipboardContent) {
      var singlePattern = /^<img src="file:\S+" \/>$/;
      var content = '';
    ...
    var pasteImage = function (editor, imageItem) {
      ...
      if (isValidDataUriImage(editor, img)) {
        ...
        if (!existingBlobInfo) {
          ...
          blobInfo_1 = blobCache.create(id, file, base64, name_1, filename);
          blobCache.add(blobInfo_1);
        } else {
          blobInfo_1 = existingBlobInfo;
        }
        img.onload = function () {
          var clientWidth = editor.editorContainer.clientWidth;
          var width = img.width > clientWidth * 0.8 ? ' width="80%"' : '';
          pasteHtml$1(editor, '<img src="' + blobInfo_1.blobUri() + '"' + width + '">', false);
        };
      } else {
    ...
    var getImagesFromDataTransfer = function (editor, dataTransfer, clipboardContent) {
      var items = dataTransfer.items ? filter(map(from$1(dataTransfer.items), function (item) {
        var itemFile = item.getAsFile();
        if (itemFile && 'image.png' === itemFile.name) {
          return null;
        }
        return itemFile;
      }), function (item) {
    ...
    var getVideosFromDataTransfer = function (dataTransfer) {
      var items = dataTransfer.items ? map(from$1(dataTransfer.items), function (item) {
        return item.getAsFile();
      }) : [];
      var files = dataTransfer.files ? from$1(dataTransfer.files) : [];
      var videos = filter(items.length > 0 ? items : files, function (file) {
        return file && /^video\/(mp4|quicktime|x-ms-wmv|x-msvideo|x-matroska|x-flv)$/.test(file.type);
      });
      return videos;
    };
    var pasteImageOrVideoData = function (editor, e, rng) {
      var dataTransfer = isClipboardEvent(e) ? e.clipboardData : e.dataTransfer;
      if (getPasteDataImages(editor) && dataTransfer) {
        var clipboardContent = null;
        if (isClipboardEvent(e)) {
          clipboardContent = getClipboardContent(editor, e);
        }
      ...
      var filePickerCallback = getFilePickerCallback(editor);
      if (filePickerCallback && dataTransfer) {
        var files = getVideosFromDataTransfer(dataTransfer);
        if (files.length > 0) {
          e.preventDefault();
          filePickerCallback(function (data) {
            var videoTemplateCallback = getVideoTemplateCallback(editor);
            if (data.preview_url && videoTemplateCallback) {
    ...
    var registerEventHandlers = function (editor, pasteBin, pasteFormat) {
      ...
      function insertClipboardContent(editor, clipboardContent, isKeyBoardPaste, plainTextMode, internal) {
        ...
        if (plainTextMode) {
          var urlReg = /(((?:[a-z][a-z0-9+.-]{1,31}:)?)((?:(?:\/\/)(?:\S+(?::\S*)?@)?(?:(?:1\d\d|2[01]\d|22[0-3]|[1-9]\d?)(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:1\d\d|2[0-4]\d|25[0-4]|[1-9]\d?))|(?![-_])(?:[-\w\xa1-\xff]{0,63}[^-_]\.)+(?:[a-zA-Z\xa1-\xff]{2,}\.?))(?::\d{2,5})?(?:[/?#][^\s<>\x00-\x1f\u4e00-\u9fa5"()]*)?)|(?:[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*)))/ig;
          if (content.match(urlReg) === null) {
            pasteText(editor, content);
          } else {
            var encodedText = editor.dom.encode(content).replace(/\r\n/g, '\n');
            var normalizedText = normalizeWhitespace(editor, encodedText);
            var textHtml = convert(normalizedText, getForcedRootBlock(editor), getForcedRootBlockAttrs(editor));
            var linkHtml = textHtml.replace(urlReg, '<a href="$1" target="_blank">$1</a>');
            pasteHtml$1(editor, linkHtml, internal);
          }
        } else {
      ...
      editor.on('paste', function (e) {
        ...
        if (!hasHtmlOrText(clipboardContent) || hasWXWorkImage(clipboardContent)) {
          if (pasteImageOrVideoData(editor, e, getLastRng())) {
            pasteBin.remove();
            return;
          }
        }
        if ((isPasteSingleImage(clipboardContent) || !hasHtmlOrText(clipboardContent)) && pasteImageData(editor, e, getLastRng())) {
          pasteBin.remove();
          return;
        }
        if (!isKeyboardPaste) {
    ...
    var Clipboard = function (editor, pasteFormat) {
      ...
      return {
        ...
        pasteImageOrVideoData: function (e, rng) {
          return pasteImageOrVideoData(editor, e, rng);
        },
    ...
    var setDragTip = function (editor, clipboard, state) {
      var dragTip = document.querySelector('#editor-dragtip');
      if (dragTip) {
    ...
    function removeWebKitStyles(editor, content, internal, isWordHtml) {
      ...
      if (webKitStyles) {
        ...
        content = content.replace(/(<[^>]+) style="([^"]*)"([^>]*>)/gi, function (all, before, value, after) {
          ...
          var result = /filetype="tx-doc"/.test(before);
          if (outputStyles && !result) {
